...
import type { Authorizer, Permission, RoleAssignments } from './fga.js'
...
export function registerRoutes(app: FastifyInstance, deps: RouteDeps): void {
  ...
  const record = (
    game: StoredGame,
    kind: MoveKind | 'game_start' | 'game_end',
    actor: string,
  ...
  const handleMove = async (
    ...
    apply: (game: StoredGame, identity: AgentIdentity) => void,
  ) => {
  ...
  app.post<{ Params: { id: string }; Body: { team: Team; word: string; count: number } }>(
    '/games/:id/clue',
    async (req, reply) => {
      const { status, body } = await handleMove(
        ...
        'can_give_clue',
        (game) => {
          game.state = giveClue(game.state, req.body.team, req.body.word, req.body.count)
        },
      )
  ...
  app.post<{ Params: { id: string }; Body: { team: Team; word: string } }>(
    '/games/:id/guess',
    async (req, reply) => {
      const { status, body } = await handleMove(
        req.params.id,
        ...
        'can_guess',
        (game) => {
          game.state = guess(game.state, req.body.team, req.body.word)
        },
      )
      return reply.status(status).send(body)
    },
  ...
  app.post<{ Params: { id: string }; Body: { team: Team } }>(
    '/games/:id/pass',
    async (req, reply) => {
      const { status, body } = await handleMove(
        req.params.id,
        ...
        'can_guess',
        (game) => {
          game.state = pass(game.state, req.body.team)
        },
      )
      return reply.status(status).send(body)
    },
  ...
  app.get<{ Params: { id: string } }>('/games/:id/key', async (req, reply) => {
    ...
    if (!allowed) {
      game.events.push({
        kind: 'guess', // key-peek attempts logged under the closest kind; refine in week 2
        actor: identity.did,
        outcome: 'denied_authz',
        detail: `${identity.did} attempted to view the key card`,
