...
No IdP to configure — the engine verifies agent tokens by resolving their
DIDs. `ENGINE_DID` defaults to the referee DID in `.env.example`, and
`DOMAIN=beckitrue.com` is already the default. Leave `FGA_STORE_ID` and
`FGA_MODEL_ID` blank — the `fga-init` service creates the OpenFGA store and
writes the auth model automatically on first `docker compose up`.
...
OpenFGA tuples. The referee's DID is also the token audience (`ENGINE_DID`).
